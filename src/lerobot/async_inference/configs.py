...
def get_aggregate_function(name: str) -> Callable[[torch.Tensor, torch.Tensor], torch.Tensor]:
    ...
    if name not in AGGREGATE_FUNCTIONS:
        ...
        raise ValueError(
            f"Unknown aggregate function '{name}'. Available: {available}")
...
class PolicyServerConfig:
    ...
    host: str = field(default="localhost", metadata={
                      "help": "Host address to bind the server to"})
    port: int = field(default=8080, metadata={
                      "help": "Port number to bind the server to"})

    # Timing configuration
    fps: int = field(default=DEFAULT_FPS, metadata={
                     "help": "Frames per second"})
    inference_latency: float = field(
        default=DEFAULT_INFERENCE_LATENCY, metadata={
            "help": "Target inference latency in seconds"}
    )

    obs_queue_timeout: float = field(
        default=DEFAULT_OBS_QUEUE_TIMEOUT, metadata={
...
class RobotClientConfig:
    ...
    actions_per_chunk: int = field(
        metadata={"help": "Number of actions per chunk"})

    # Task instruction for the robot to execute (e.g., 'fold my tshirt')
    task: str = field(default="", metadata={
                      "help": "Task instruction for the robot to execute"})

    # GR00T-specific: embodiment tag for the robot (e.g., 'new_embodiment', 'so100', 'gr1')
    embodiment_tag: str | None = field(
        default=None,
        metadata={
            "help": "Embodiment tag for GR00T policy (e.g., 'new_embodiment', 'so100', 'gr1')"}
    )

    # Network configuration
    server_address: str = field(default="localhost:8080", metadata={
                                "help": "Server address to connect to"})

    # Device configuration
    policy_device: str = field(default="cpu", metadata={
